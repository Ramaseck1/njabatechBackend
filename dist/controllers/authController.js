...
class AuthController {
    ...
    static async forgotPasswordGIE(req, res) {
        try {
            ...
            if (gie) {
                const code = await resetCodeStore_1.ResetCodeStore.create(gie.id, email);
    ...
    static async verifyResetCodeGIE(req, res) {
        try {
            ...
            const entry = await resetCodeStore_1.ResetCodeStore.verify(email, code);
            if (!entry) {
                res.status(400).json({
                    success: false,
    ...
    static async resetPasswordGIE(req, res) {
        try {
            ...
            const entry = await resetCodeStore_1.ResetCodeStore.canReset(email);
            if (!entry) {
                res.status(400).json({
                    success: false,
            ...
            await resetCodeStore_1.ResetCodeStore.delete(email);
            res.json({
                success: true,
