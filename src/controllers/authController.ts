...
data.telephone = cleanTelephone;
  ...
  static async forgotPasswordGIE(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (gie) {
        const code = await ResetCodeStore.create(gie.id, email); // ← await ajouté
  ...
  static async verifyResetCodeGIE(req: Request, res: Response): Promise<void> {
    try {
      ...
      const entry = await ResetCodeStore.verify(email, code); // ← await ajouté
      if (!entry) {
        res.status(400).json({
          success: false,
 ...
 static async resetPasswordGIE(req: Request, res: Response): Promise<void> {
    try {
      ...
      const entry = await ResetCodeStore.canReset(email); // ← await ajouté
      if (!entry) {
        res.status(400).json({
          success: false,
      ...
      await ResetCodeStore.delete(email); // ← await ajouté (methode devenue async)
